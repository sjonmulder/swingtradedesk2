...
**Manual watchlist** — type in tickers yourself, or pick one of the four
preset lists (Mega Cap, Semis, Index/ETF, High Beta).

**Discover by cap / volume** — instead of naming tickers, set a minimum
market cap (small/mid/large/mega cap presets) and a minimum average daily
volume, plus a cap on how many results to pull. This calls FMP's
`/api/v3/stock-screener` endpoint to find matching tickers first, then runs
the same signal engine on whatever it finds — so you can go from "show me
large-cap, liquid names" to a ranked list of CALL/PUT signals without typing
a single symbol.

Note: FMP's `isEtf`/`isFund` filters are used to keep the discover mode
focused on individual stocks rather than index funds, and results are capped
(30 max) since each discovered ticker costs 2 more API requests (history +
quote) on top of the screener call itself.
...
```
app/
  api/screen/route.ts   # GET /api/screen?symbols=AAPL,MSFT — manual mode
                          # GET /api/screen?discover=true&marketCapMin=...&volumeMin=... — discover mode
